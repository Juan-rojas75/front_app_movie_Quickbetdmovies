...
export default function Navbar() {
  ...
  const Links = [
    { name: "Populares", href: "/home/populares", logged:false },
    { name: "Favorites", href: "/home/favorites", logged:true },
  ];

  const nav =  Links.map((link) => {
    if (link.logged && token || !link.logged) {
      return (
        <Link key={link.name} href={link.href}>
          <p className="hidde md:block">{link.name}</p>
        </Link>
  ...
  return (
    <div className=" text-base">
      ...
      <nav className="flex flex-row justify-between gap-4 px-20 bg-black h-16 items-center">
